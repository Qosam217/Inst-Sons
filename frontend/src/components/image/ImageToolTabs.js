...
export default function ImageToolTabs({ activeTab, onSelectTab, disabled = false }) {
  const tabs = [
    {
      id: 'compress',
      label: 'Kompres Gambar',
      badge: 'Smart Optimizer',
      icon: Minimize2,
      description: 'Kecilkan ukuran file foto & gambar',
    },
    {
      id: 'convert',
      ...
      icon: RefreshCw,
      description: 'Ubah ke WebP, PNG, JPG, AVIF, GIF',
    },
  ];

  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 p-1.5 bg-slate-900/90 border border-slate-800 rounded-2xl">
      {tabs.map((tab) => {
        const Icon = tab.icon;
        ...
        return (
          <button
            ...
            className={clsx(
              ...
              isActive
                ? 'bg-amber-500/15 border border-amber-500/40 shadow-sm text-white'
